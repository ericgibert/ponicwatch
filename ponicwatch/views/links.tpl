...
<tr><td style="text-align:center">{{ "X" if row[0]>0 else "-" }}</td>
    ...
    <td>{{ controller.get_pwo("sensor",row[1]) if row[1] else "-" }}</td>
    <td>{{ controller.get_pwo("switch",row[2]) if row[2] else "-"  }}</td>
    <td>{{ controller.get_pwo("hardware",row[3]) if row[3] else "-"  }}</td>
    <td>{{ controller.get_pwo("interrupt",row[5]) if row[5] else "-"  }}</td>
    <td>{{row[4]}}</td>
% end
</table>
</body>
