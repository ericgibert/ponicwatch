...
class IC_MCP23017(object):
    ...
    def __init__(self, pig, address, num_gpios=16, busnum=1):
        ...
        self.i2c = pig.i2c_open(busnum, address)
        self.address = address
        self.num_gpios = num_gpios

        # set defaults
        self.i2c.i2c_write_byte(IC_MCP23017.IODIRA, 0xFF)  # all inputs on port A
        self.i2c.i2c_write_byte(IC_MCP23017.IODIRB, 0xFF)  # all inputs on port B
        self.i2c.i2c_write_byte(IC_MCP23017.GPIOA, 0x00)  # output register to 0
        self.i2c.i2c_write_byte(IC_MCP23017.GPIOB, 0x00)  # output register to 0

        # read the current direction of all pins into instance variable
        # self.direction used for assertions in a few methods methods
        self.direction = self.i2c.i2c_read_byte(IC_MCP23017.IODIRA)
        self.direction |= self.i2c.i2c_read_byte(IC_MCP23017.IODIRB) << 8

        # disable the pull-ups on all ports
        self.i2c.i2c_write_byte(IC_MCP23017.GPPUA, 0x00)
        self.i2c.i2c_write_byte(IC_MCP23017.GPPUB, 0x00)

        # clear the IOCON configuration register, which is chip default
        self.i2c.i2c_write_byte(IC_MCP23017.IOCON, 0x00)

        ##### interrupt defaults
        # disable interrupts on all pins by default
        self.i2c.i2c_write_byte(IC_MCP23017.GPINTENA, 0x00)
        self.i2c.i2c_write_byte(IC_MCP23017.GPINTENB, 0x00)
        # interrupt on change register set to compare to previous value by default
        self.i2c.i2c_write_byte(IC_MCP23017.INTCONA, 0x00)
        self.i2c.i2c_write_byte(IC_MCP23017.INTCONB, 0x00)
        # interrupt compare value registers
        self.i2c.i2c_write_byte(IC_MCP23017.DEFVALA, 0x00)
        self.i2c.i2c_write_byte(IC_MCP23017.DEFVALB, 0x00)
    ...
    def _readAndChangePin(self, register, pin, value, curValue=None):
        ...
        if curValue is None:
            curValue = self.i2c.i2c_read_byte(register)
        ...
        self.i2c.i2c_write_byte(register, newValue)
        return newValue
    ...
    def output(self, pin, value):
        ...
        # if the pin is < 8, use register from first bank
        if (pin < 8):
            self.outputvalue = self._readAndChangePin(IC_MCP23017.GPIOA, pin, value, self.i2c.i2c_read_byte(IC_MCP23017.OLATA))
        else:
            # otherwise use register from second bank
            # readAndChangePin accepts pin relative to register though, so subtract
            self.outputvalue = self._readAndChangePin(IC_MCP23017.GPIOB, pin - 8, value, self.i2c.i2c_read_byte(IC_MCP23017.OLATB))
    ...
    def input(self, pin):
        ...
        """
        assert 0 <= pin < self.num_gpios, "Pin number %s is invalid, only 0-%s are valid" % (pin, self.num_gpios)
        regValue = self.i2c.i2c_read_byte(IC_MCP23017.GPIOA if pin<8 else IC_MCP23017.GPIOB)
        if pin >= 8: pin -= 8
        return int(regValue & (1 << pin) != 0)
        # value = 0
        # # reads the whole register then compares the value of the specific pin
        # if (pin < 8):
        #     regValue = self.i2c.i2c_read_byte(IC_MCP23017.GPIOA)
        #     if regValue & (1 << pin) != 0: value = 1
        # else:
        #     regValue = self.i2c.i2c_read_byte(IC_MCP23017.GPIOB)
    ...
    def configSystemInterrupt(self, mirror, intpol):
        ...
        registerValue = self.i2c.i2c_read_byte(IC_MCP23017.IOCON)
        # set mirror bit
        ...
        # set ODR pin
        self.i2c.i2c_write_byte(IC_MCP23017.IOCON, registerValue)
    ...
    def _readInterruptRegister(self, port):
        ...
        interrupted = self.i2c.i2c_read_byte(IC_MCP23017.INTFA if port==0 else IC_MCP23017.INTCAPB)
        if interrupted:
            pin = int(math.log(interrupted, 2))  # first non 0 pin having triggered the interrupt
            # get the value of the pin
            value_register = self.i2c.i2c_read_byte(IC_MCP23017.INTCAPA if port==0 else IC_MCP23017.INTCAPB)
        ...
        # pin = None
        # if port == 0:
        #     interruptedA = self.i2c.i2c_read_byte(IC_MCP23017.INTFA)
        #     if interruptedA != 0:
        #         pin = int(math.log(interruptedA, 2))
        #         # get the value of the pin
        #         valueRegister = self.i2c.i2c_read_byte(IC_MCP23017.INTCAPA)
        #         if valueRegister & (1 << pin) != 0: value = 1
        # else:
        #     interruptedB = self.i2c.i2c_read_byte(IC_MCP23017.INTFB)
        #     if interruptedB != 0:
        #         pin = int(math.log(interruptedB, 2))
        #         # get the value of the pin
        #         valueRegister = self.i2c.i2c_read_byte(IC_MCP23017.INTCAPB)
        #         if valueRegister & (1 << pin) != 0: value = 1
    ...
    def clearInterrupts(self):
        if self.i2c.i2c_read_byte(IC_MCP23017.INTFA) > 0 or self.i2c.i2c_read_byte(IC_MCP23017.INTFB) > 0:
            iterations = 3
            count = 1
            # loop to check multiple times to lower chance of false positive
            while count <= iterations:
                if self.i2c.i2c_read_byte(IC_MCP23017.INTFA) == 0 and self.i2c.i2c_read_byte(IC_MCP23017.INTFB) == 0:
                    return 0
                else:
                    time.sleep(.5)
                    count += 1
    ...
    def cleanup(self):
        ...
        self.i2c.i2c_write_byte(IC_MCP23017.GPIOA, 0x00)
        self.i2c.i2c_write_byte(IC_MCP23017.GPIOB, 0x00)
        # disable the pull-ups on all ports
        self.i2c.i2c_write_byte(IC_MCP23017.GPPUA, 0x00)
        self.i2c.i2c_write_byte(IC_MCP23017.GPPUB, 0x00)
        # clear the IOCON configuration register, which is chip default
